...
| `opusclip_list_clips` | collect candidates | rank, score, sub-scores, title, description, hashtags, duration, preview + **thumbnail** URLs, and the project `stage` (empty list + in-progress stage = not ready yet). **Exact key names and stage values not yet observed** (no project existed on 2026-09-23; a missing project returns the error `Project <id> not found.`). `candidate upsert` accepts snake_case, camelCase and REST-style keys, bare arrays and the MCP text envelope, and classifies stages by keyword (`fail/error/cancel` → failed, `complete/done/finish/ready/…` → done, anything else → in progress). Narrow it after the first live project. |
