...
- **Verified 2026-09-23:** a manually started cloud run on the empty live database reported "Needs you: nothing" (0 campaigns/jobs/candidates; OpusClip usage reconciled at 0/900). `attention notify` and `notify` refuse until `NOTIFY_WEBHOOK_URL` is set on Render (task 12's pending webhook), so that report stayed in the session.
