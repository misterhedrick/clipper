...
- **OpusClip brand templates are made by hand.** The API can only list them. The account's **default template is kept clean** (no logo) and serves every campaign without logo rules; campaigns that require a logo get their own template (see the operator's onboarding guide). The default used to carry the MW4 logo, which put it on the first Charlie Berens clips (2026-09-24).
